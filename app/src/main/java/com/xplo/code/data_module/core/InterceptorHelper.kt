...
object InterceptorHelper {
    ...
    fun init(context: Context) {
        com.xplo.code.data_module.core.InterceptorHelper.initStetho(context)
        //initFlipper(context)
        // okhttp, curl doesn't need init
    }

    fun initStetho(context: Context) {
        if (!com.xplo.code.data_module.core.InterceptorHelper.isStethoEnabled) return
...
//    }
    ...
    fun initChucker(context: Context) {
        if (!com.xplo.code.data_module.core.InterceptorHelper.isChuckerEnabled) return
        com.xplo.code.data_module.core.InterceptorHelper.chuckerCollector = ChuckerCollector(
            context = context,
            showNotification = true,
            retentionPeriod = RetentionManager.Period.ONE_HOUR
        )

        com.xplo.code.data_module.core.InterceptorHelper.chuckerInterceptor =
            com.xplo.code.data_module.core.InterceptorHelper.createChuckerInterceptor(
                context,
                com.xplo.code.data_module.core.InterceptorHelper.chuckerCollector
            )
    }
